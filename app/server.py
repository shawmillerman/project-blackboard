...
def tier2_feedback_suggest_post(request: Request, payload: FeedbackSuggestRequest):
    ...
    # Default to BA101, constrain everything else explicitly
    ...
    assignment_id=payload.assignment_id,
        ...
        elapsed_ms = int((time.time() - t0) * 1000)
        logger.exception(
            "tier2_feedback_suggest failed request_id=%s ip=%s course=%s assignment_type=%s category=%s severity=%s elapsed_ms=%s",
            request_id,
            request.client.host if request.client else None,
            metadata.get("course"),
            metadata.get("assignment_type"),
